...
void i2c_configure_master(unsigned char slave_addr) {
    ...
    // Configred I2C Master mode
    SSPCON1bits.SSPM = 0x08; // b1000
    SSPCON1bits.SSPEN = 1;

    SSPSTAT = SLEW_OFF; // b1000_0000
...
unsigned char i2c_master_send(unsigned char length, unsigned char *msg) {
    ...
    // Set the msg to the i2c buffer
    for (int i = 0; i < length; i++) {
        ic_ptr->outbuffer[i] = msg[i];
    }
...
void i2c_master_int_handler() {
    switch (ic_ptr->status) {
        ...
        {
            // Can't get ack bit from MARK III, need to check
        ...
        case I2C_MASTER_RECV_SET_RCEN:
        {
            if (SSPCON2bits.ACKSTAT == 0) {
                ...
                LATBbits.LATB0 = SSP1STATbits.BF;
